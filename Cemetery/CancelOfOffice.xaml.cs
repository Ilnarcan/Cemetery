...
{
    /// <summary>
    /// Логика взаимодействия для CancelOfOffice.xaml
    /// </summary>
    public partial class CancelOfOffice
    {
        ...
        public CancelOfOffice(string y, string m, string d, int md, string per)
        {
            _year = y;
            _month = m;
            _day = d;
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            Label4.Content = "Списание канцелярских товаров " + _period;
            if (!String.IsNullOrEmpty(_year) && !String.IsNullOrEmpty(_month) && !String.IsNullOrEmpty(_day))
            {
                ...
                string sqlquer = "SELECT ID,date,cheque,name,unit,number,sum,total FROM COC WHERE ";
                ...
                foreach (DataRow record in dtmem.Rows)
                {
                    _myTable.Rows.Add(new object[]
                    {
                       ...
                       CryptDec.NewForm(record["unit"].ToString()), CryptDec.NewForm(record["number"].ToString()),
                       CryptDec.NewForm(record["sum"].ToString()), CryptDec.NewForm(record["total"].ToString())
                    }); 
        ...
        private void button2_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                if (rowItem != null)
                {
                    ...
                    connection.Open();
                    var command = new SQLiteCommand("DELETE FROM COC WHERE ID = '" + strNum + "'", connection);
        ...
        private void button3_Click(object sender, RoutedEventArgs e)
        {
            ...
            string headerName = "Списание канцелярских товаров " + _period;
        ...
        private void button4_Click(object sender, RoutedEventArgs e)
        {
            ...
            var datecr = CryptDec.Encrypt(date, "Passpord11", "Password22", "SHA1", 2,
                                                 "16CHARSLONG12345");
            var suppliercr = CryptDec.Encrypt(supplier, "Passpord11", "Password22", "SHA1", 2,
                                                 "16CHARSLONG12345");
            var namecr = CryptDec.Encrypt(name, "Passpord11", "Password22", "SHA1", 2,
                                                 "16CHARSLONG12345");
            var unitcr = CryptDec.Encrypt(unit, "Passpord11", "Password22", "SHA1", 2,
                                                 "16CHARSLONG12345");
            var numbercr = CryptDec.Encrypt(number.ToString(CultureInfo.InvariantCulture), "Passpord11", "Password22", "SHA1", 2,
                                                 "16CHARSLONG12345");
            var summacr = CryptDec.Encrypt(summa.ToString(CultureInfo.InvariantCulture), "Passpord11", "Password22", "SHA1", 2,
                                                 "16CHARSLONG12345");
            var totalcr = CryptDec.Encrypt(total.ToString(CultureInfo.InvariantCulture), "Passpord11", "Password22", "SHA1", 2,
                                                 "16CHARSLONG12345");
            ...
            var command2 = new SQLiteCommand("SELECT Count(*) FROM COC", connection);
            ...
            {
                numberdead++;
                var command3 = new SQLiteCommand("SELECT Count(*) FROM COC WHERE ID = '" + numberdead + "'", connection);
            ...
            string sqlquery = "INSERT INTO COC(ID,year,month,day,date,cheque,name,unit,number,sum,total) VALUES ('" +
