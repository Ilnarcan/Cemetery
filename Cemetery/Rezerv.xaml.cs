...
namespace Cemetery
{
    ...
    public partial class Rezerv
    {
        ...
        public Rezerv(string y, string m, string d, int md, string per)
        {
            _year = y;
            _month = m;
            _day = d;
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            Label4.Content = "Захоронение в резерв " + _period;
            if (!String.IsNullOrEmpty(_year) && !String.IsNullOrEmpty(_month) && !String.IsNullOrEmpty(_day))
            {
                ...
                string sqlquer = "SELECT ID,datez,cheque,total,sod,name,dog,dates,srp FROM REZ WHERE ";
                ...
                foreach (DataRow record in dtmem.Rows)
                {
                    ...
                    {
                       (Int64) record["ID"], CryptDec.NewForm(record["datez"].ToString()), 
                       CryptDec.NewForm(record["cheque"].ToString()), CryptDec.NewForm(record["total"].ToString()), 
                       CryptDec.NewForm(record["sod"].ToString()), CryptDec.NewForm(record["name"].ToString()),
                       CryptDec.NewForm(record["dog"].ToString()), CryptDec.NewForm(record["dates"].ToString()),
                       CryptDec.NewForm(record["srp"].ToString())
                    });
                ...
                dt.Columns[1].ColumnName = "Дата захор";
                dt.Columns[2].ColumnName = "№ квит дата";
                dt.Columns[3].ColumnName = "Сумма";
                dt.Columns[4].ColumnName = "Содержание услуг";
                dt.Columns[5].ColumnName = "ум ФИО";
                dt.Columns[6].ColumnName = "№ договора";
                dt.Columns[7].ColumnName = "Дата сост";
                dt.Columns[8].ColumnName = "Уч ряд место";
        ...
        private void dataGrid1_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (rowView != null)
            {
                ...
                DatePicker2.Text = rowView[7].ToString();
                TextBox3.Text = rowView[8].ToString();
            }
        }
        ...
        private void button1_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                if (rowItem != null)
                {
                    ...
                    var datezD = CryptDec.BackToForm(date);
                    var chequeD = CryptDec.BackToForm(TextBox2.Text);
                    var totalD = CryptDec.BackToForm(TextBox1.Text);
                    var sodD = CryptDec.BackToForm(TextBox4.Text);
                    var nameD = CryptDec.BackToForm(TextBox6.Text);
                    var dogD = CryptDec.BackToForm(TextBox7.Text);
                    var datesD = CryptDec.BackToForm(date2);
                    var srpD = CryptDec.BackToForm(TextBox3.Text);
                    if (!(File.Exists(databaseNamecrypt)))
                    {
                        Environment.Exit(0);
                    }
                    ...
                    var command =
                        new SQLiteCommand(
                            ...
                            "dog = '" + dogD + "'," +
                            "dates = '" + datesD + "'," +
                            "srp = '" + srpD + "' WHERE ID = '" + strNum + "'", connection);
                    command.ExecuteNonQuery();
                    ...
                    _myTable.Rows[rowView][7] = date2;
                    _myTable.Rows[rowView][8] = TextBox3.Text;
                }
                _myTable.Columns[0].ColumnName = "ID";
                _myTable.Columns[1].ColumnName = "Дата захор";
                _myTable.Columns[2].ColumnName = "№ квит дата";
                _myTable.Columns[3].ColumnName = "Сумма";
                _myTable.Columns[4].ColumnName = "Содержание услуг";
                _myTable.Columns[5].ColumnName = "ум ФИО";
                _myTable.Columns[6].ColumnName = "№ договора";
                _myTable.Columns[7].ColumnName = "Дата сост";
                _myTable.Columns[8].ColumnName = "Уч ряд место";
        ...
        private void button2_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                if (rowItem != null)
                {
                    ...
                    connection.Open();
                    var command = new SQLiteCommand("DELETE FROM REZ WHERE ID = '" + strNum + "'", connection);
        ...
        private void button3_Click(object sender, RoutedEventArgs e)
        {
            ...
            string headerName = "Захоронения в резерв " + _period;
            ...
            currentRow.Cells.Add(new TableCell(new Paragraph(new Run("Дата захор."))));
            currentRow.Cells.Add(new TableCell(new Paragraph(new Run("№ квит,дата"))));
            currentRow.Cells.Add(new TableCell(new Paragraph(new Run("Сумма"))));
            currentRow.Cells.Add(new TableCell(new Paragraph(new Run("Содержание услуг"))));
            currentRow.Cells.Add(new TableCell(new Paragraph(new Run("Ум. ФИО"))));
            currentRow.Cells.Add(new TableCell(new Paragraph(new Run("№ договора"))));
            currentRow.Cells.Add(new TableCell(new Paragraph(new Run("Дата сост."))));
            currentRow.Cells.Add(new TableCell(new Paragraph(new Run("Уч,ряд,место"))));
        ...
        {
            string datez = DatePicker1.ToString();
            string cheque = TextBox2.Text;
            double total = 0.0;
            string sod = TextBox4.Text;
            string name = TextBox6.Text;
            string dog = TextBox7.Text;
            string dates = DatePicker2.ToString();
            string srp = TextBox3.Text;
            double totalsum = 0.0;
            string[] mem = datez.Split(' ');
            datez = mem[0];
            Int64 numberdead = 0;

            string[] mem2 = dates.Split(' ');
            dates = mem2[0];

            if (!String.IsNullOrEmpty(TextBox1.Text))
                total = Convert.ToDouble(TextBox1.Text);

            var datezcr = CryptDec.BackToForm(datez);
            var chequecr = CryptDec.BackToForm(cheque);
            var totalcr = CryptDec.BackToForm(total.ToString(CultureInfo.InvariantCulture));
            var sodcr = CryptDec.BackToForm(sod);
            var namecr = CryptDec.BackToForm(name);
            var dogcr = CryptDec.BackToForm(dog);
            var datescr = CryptDec.BackToForm(dates);
            var srpcr = CryptDec.BackToForm(srp);
            ...
            var command2 = new SQLiteCommand("SELECT Count(*) FROM REZ", connection);
            ...
            {
                numberdead++;
                var command3 = new SQLiteCommand("SELECT Count(*) FROM REZ WHERE ID = '" + numberdead + "'", connection);
            ...
            string sqlquery = "INSERT INTO REZ(ID,year,month,day,datez,cheque,total,sod,name,dog,dates,srp) VALUES ('" +
                              numberdead + "','" + DateTime.Now.Year + "','" + DateTime.Now.Month + "','" + DateTime.Now.Day + "','" + datezcr + "','" +
                              chequecr + "','" + totalcr + "','" + sodcr + "','" + namecr + "','" + dogcr + "','" + datescr + "','" + srpcr + "')";
            ...
            _myTable.Rows.Add(new object[]
                {
                    numberdead.ToString(CultureInfo.InvariantCulture), datez,cheque,total.ToString(CultureInfo.InvariantCulture),sod,name,dog,dates,srp
                });
